...
reorgXcalConc<-function(saveFile, saveName){
  ...
  for(i in 1:n_comp){
    ...
    if(length(wbList)<i){
      wbList[[i]]=matrix(rep("NF",(sampleLength-6)),nrow=(sampleLength-6), ncol=1)
    }
    if(length(wbList[[i]][,1])<(sampleLength-6)){
      wbList[[i]]=matrix(rep("NF",(sampleLength-6)),nrow=(sampleLength-6), ncol=1)
    }
    wbList[[i]][(grep("NF",wbList[[i]][,1])),1]<-0
  }
  ...
  for(i in 1:(n_comp)){
    expDF[-1,i+1]<-as.numeric(unlist(wbList[[i]]))
  }
